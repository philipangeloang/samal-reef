...
interface MoaSigningFormProps {
  ownershipId: number;
}

export function MoaSigningForm({ ownershipId }: MoaSigningFormProps) {
  const router = useRouter();
  const signatureRef = useRef<SignatureCanvas>(null);
  const [signerName, setSignerName] = useState("");
  const [hasDrawnSignature, setHasDrawnSignature] = useState(false);

  // Fetch ownership details and MOA preview
  const { data: moaPreview, isLoading: isLoadingPreview } =
    api.moa.generateUnsignedPreview.useQuery({ ownershipId });

  const { data: ownership, isLoading: isLoadingOwnership } =
    api.moa.getMoaByOwnership.useQuery({ ownershipId });

  // Submit signed MOA mutation
  const submitMoa = api.moa.submitSignedMoa.useMutation({
    onSuccess: (_data) => {
      toast.success("MOA signed successfully!", {
    ...
    onError: (error) => {
      toast.error("Failed to sign MOA", {
        description: error.message ?? "An error occurred while signing the MOA",
      });
    },
  });
      ...
      <Card className="border-green-400/30 bg-gradient-to-br from-[#0d1f31]/90 to-[#0a1929]/90 backdrop-blur-sm">
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <CardTitle className="text-white">MOA Already Signed</CardTitle>
          </div>
          <CardDescription className="text-cyan-100/70">
            You signed this document on{" "}
            {ownership.moaSignedAt
              ? new Date(ownership.moaSignedAt).toLocaleDateString("en-US", {
                  year: "numeric",
                  month: "long",
                  day: "numeric",
                })
  ...
  // Error state
  if (!moaPreview || !ownership) {
    return (
      <Alert className="border-red-400/30 bg-red-400/10">
        <AlertCircle className="h-4 w-4 text-red-400" />
        <AlertDescription className="text-red-300">
          Unable to load MOA document. Please try again or contact support.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <CardContent className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="font-semibold text-white">
                  {moaPreview.ownershipDetails.unitName}
                </p>
              </div>
              <div>
                ...
                <p className="font-semibold text-white">
                  {moaPreview.ownershipDetails.percentage}%
                </p>
              </div>
              <div>
                <Label className="text-xs text-cyan-100/60">Purchase Price</Label>
                <p className="font-semibold text-white">
                  {moaPreview.ownershipDetails.purchasePrice}
                </p>
              </div>
              <div>
                ...
                <p className="font-semibold text-white">
                  {new Date(
                    moaPreview.ownershipDetails.purchaseDate,
        ...
        {/* Right Column - MOA Document Preview */}
        <div className="lg:sticky lg:top-6 lg:self-start">
          <Card className="border-cyan-400/20 bg-gradient-to-br from-[#0d1f31]/90 to-[#0a1929]/90 backdrop-blur-sm">
            <CardHeader>
              <CardTitle className="text-white">MOA Document Preview</CardTitle>
              <CardDescription className="text-cyan-100/60">
            ...
            <CardContent>
              <div className="aspect-[8.5/11] w-full overflow-hidden rounded-lg border border-cyan-400/30 bg-white">
                <embed
                  src={`data:application/pdf;base64,${moaPreview.pdfBase64}`}
